...
// TestTransportPartitionImportState exercises the ImportState path end-to-end:
// it confirms the parsed identifiers land in the "id" and "project_id" schema
...
func TestTransportPartitionImportState(t *testing.T) {
	const (
	...
	t.Run("explicit project id from suffix", func(t *testing.T) {
		r := &transportPartitionResource{client: &common.CrusoeClient{ProjectID: fallbackUUID}}
...
// TestTransportPartitionSchemaDescriptions guards the CCX-2836 convention that
// every attribute carries a description, so the published registry page is not
// blank the way it was before.
func TestTransportPartitionSchemaDescriptions(t *testing.T) {
	ctx := context.Background()

	schemaResp := &resource.SchemaResponse{}
	NewTransportPartitionResource().Schema(ctx, resource.SchemaRequest{}, schemaResp)
	if schemaResp.Diagnostics.HasError() {
		t.Fatalf("failed to build schema: %v", schemaResp.Diagnostics)
	}
