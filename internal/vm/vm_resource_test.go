...
// TestVMImportState exercises the ImportState path end-to-end: it confirms the
// parsed identifiers land in the "id" and "project_id" schema attributes, that
// the client project ID is used as a fallback, and that an invalid identifier
// surfaces a diagnostic. The parsing logic itself is covered by
// TestParseResourceIdentifiers in the common package.
func TestVMImportState(t *testing.T) {
	const (
	...
	t.Run("explicit project id from suffix", func(t *testing.T) {
		r := &vmResource{client: &common.CrusoeClient{ProjectID: fallbackUUID}}
