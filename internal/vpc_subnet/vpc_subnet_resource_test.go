...
// TestVPCSubnetImportState exercises the full ImportState path end-to-end: it
...
// invalid identifier surfaces a diagnostic. The other resources changed for
// CCX-101 use the identical wrapper around common.ParseResourceIdentifiers,
// whose parsing logic is covered by TestParseResourceIdentifiers.
func TestVPCSubnetImportState(t *testing.T) {
	const (
	...
	t.Run("explicit project id from suffix", func(t *testing.T) {
		r := &vpcSubnetResource{client: &common.CrusoeClient{ProjectID: fallbackUUID}}
		resp := newImportStateResponse(ctx, t, r.Schema)

		r.ImportState(ctx, resource.ImportStateRequest{ID: subnetUUID + "," + projectUUID}, resp)
		...
		if gotID.ValueString() != subnetUUID {
			t.Errorf("id = %q, want %q", gotID.ValueString(), subnetUUID)
		}
	...
	t.Run("falls back to client project id", func(t *testing.T) {
		...
		r.ImportState(ctx, resource.ImportStateRequest{ID: subnetUUID}, resp)
