variables {
  name_prefix = "tf-test-transport-networks-"
  ib_vm = {
    slices   = 8
...
run "validate_create_transport_network_vm" {
  command = apply
  ...
  assert {
    condition     = crusoe_transport_partition.my_partition.id != null
    error_message = "The transport partition was not created successfully, as its ID is null."
  }
  ...
  assert {
    condition = alltrue([
      ...
      vm.host_channel_adapters[0].transport_partition_id == crusoe_transport_partition.my_partition.id
    ])
    error_message = "Failed VM partition verification - expected: ${crusoe_transport_partition.my_partition.id}, actual: ${join("; ", [
      for vm in crusoe_compute_instance.my_vms :
      "${vm.name}: ${vm.host_channel_adapters[0].transport_partition_id}"
      if vm.host_channel_adapters[0].transport_partition_id != crusoe_transport_partition.my_partition.id
    ])}"
  }
